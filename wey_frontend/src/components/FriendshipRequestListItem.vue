<template>
  <div
    class="max-w-sm px-3 bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700"
  >
    ...
    <div class="flex flex-col items-center pb-10">
      ...
      <h3 class="mb-1 text-xl font-medium text-gray-900 dark:text-white">
        {{ friendship?.created_by.name }}
      </h3>
      <span class="text-sm text-gray-500 dark:text-gray-400">Visual Designer</span>
      <div class="flex mt-4 space-x-3 lg:mt-6">
        <button
          @click="addAsFriend(friendship.id)"
          href="#"
...
export default {
  ...
  setup(props, unshift) {
    const addAsFriend = async (friendship_id) => {
      try {
        const response = await UserService.handleFriendRequest(friendship_id, "accepted");
        console.log("handler frends request");
        console.log(response)
        if (response.message == "accepted") {
          props.friends.unshift(response.friend);

          const index = props.friendshipRequests.findIndex(
            (obj) => obj.id === response.friendshipRequest.id
          );

          if (index !== -1) {
            props.friendshipRequests.splice(index, 1); // Supprimer un élément
          }
        }
      } catch (error) {
        console.log(error);
